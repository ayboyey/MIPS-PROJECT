...
.data
	...
	prompt : .asciiz "Please enter four integers seperated by an enter key.\n" 	
	values_propmpt : .asciiz "Values of a, b, c, d, e are :\n"
	result_prompt : .asciiz "\nThe result is : \n"
.text	
main:
	# Prompt the values.
	li $v0, 4		# syscall for printing a string
	la $a0, prompt		# loading the prompt from data section $a0
	syscall 
	...
	add $t5, $t0, $t1	#  (a + b) in $t5
	div $t6, $t5, $t2	#  (a + b) / c in $t6
	sub $t7, $t3, $t4 	#  (d-e) in $t7
	sub $s0, $t6, $t7	#  result of equation in $s0 - (a+b)/c - (d-e) 
